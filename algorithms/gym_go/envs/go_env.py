...
class GoEnv(gym.Env):
    ...
    def __init__(self, size, komi=0, reward_method='real'):
        ...
        self.action_space = gym.spaces.Discrete(
            gogame.action_size(self.state_))
        self.done = False
    ...
    def render(self, mode='terminal'):
        if mode == 'terminal':
            ...
            def on_draw():
                ...
                rendering.draw_grid(batch, delta, self.size,
                                    lower_grid_coord, upper_grid_coord)

                # info on top of the board
                rendering.draw_info(
                    batch, window_width, window_height, upper_grid_coord, self.state_)

                # Inform user what they can do
                rendering.draw_command_labels(
                ...
                rendering.draw_pieces(
                    batch, lower_grid_coord, delta, piece_r, self.size, self.state_)
